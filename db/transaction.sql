CREATE TABLE transaction (
    ...
    cash_transaction BOOLEAN NOT NULL, -- is this a cash transaction? TRUE if yes, FALSE if no.
    amount NUMERIC(15, 2)  NOT NULL,
    CONSTRAINT non_zero_amount CHECK(@amount > 0.00), -- value of transaction - positive values mean it's a deposit, negative mean it's a withdrawal. The @ operator in the CHECK clause is a Postgres Operator for getting absolute value of numeric.
    ...
    CONSTRAINT card_required_for_cash CHECK((card IS NOT NULL AND cash_transaction = TRUE) OR (cash_transaction = FALSE)) -- constraint only allows cash transactions to happen if an active card is present. The OR conjunctions ensure that a card can be present with transfers but is not required.
